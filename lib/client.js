  ...
  factory: (require) => {
    ...
    function TimelineOverlay({ useSessions, loadOlder, embedded }) {
      ...
      const userEvents = useMemo(() => {
        ...
        return events.filter((ev) => ev.type === "user/message" && ev.sourceKind === "user" && (ev.preview || "").trim() !== "" && !/^可能相关的记忆|^相关记忆|^【记忆|meow-memory|记忆，仅供参考/i.test(ev.preview || ""));
      }, [sessionState.data]);
      ...
      const jumpToUserMessage = useCallback(
        async (ev) => {
          ...
          const needle = (ev.text || ev.preview || "").trim().slice(0, 80);
          const findTargetByText = () => {
            ...
            for (const row of rows) {
              ...
              if (text.length >= 4 && text.startsWith(needle.slice(0, 60))) return row;
            }
            return null;
          };
          let target = findTarget();
          if (!target) target = findTargetByText();
          if (target) {
          ...
          // 宿主契约（dsh-client-runtime session.loadOlder）：
          // - async 函数所有路径都 resolve undefined，无法区分「执行中/节流跳过/无更多」；
          // - hasMore=false 时「加载更早」按钮保持存在但 disabled。
          // 因此：disabled 按钮 = 明确无更多（立即退出）；loadOlder 包超时防止 RPC 挂起导致无限转圈。
          ...
          const findLoadOlderButton = () => {
            ...
            return buttons.find((b) => !b.disabled && /加载更早|加载更多|Load earlier|Load more/.test((b.textContent || "").trim())) || null;
          };
          const findLoadOlderButtonAny = () => {
            const buttons = Array.from(chatButtonRoot.querySelectorAll("button"));
          ...
          const withTimeout = (p, ms) =>
            Promise.race([Promise.resolve(p), new Promise((resolve) => setTimeout(() => resolve(undefined), ms))]);

          console.log("[conversation-timeline] jump start", {
            key,
            hasMessageId: !!ev.messageId,
            needle: needle.slice(0, 60),
            preview: (ev.preview || "").slice(0, 60),
            rows: countRows(),
          });
          ...
          try {
            ...
            while (Date.now() - startedAt < MAX_WAIT) {
              if (typeof loadOlder === "function" && currentId) {
                try { await withTimeout(loadOlder(currentId), 5000); } catch (err) { /* 忽略，继续轮询 */ }
              }
              await wait(250);
              target = findTarget();
              if (!target) target = findTargetByText();
              ...
              const now = countRows();
              const anyBtn = findLoadOlderButtonAny();
              if (now > lastCount) {
                lastCount = now;
                idleRounds = 0;
              } else {
              ...
              console.log("[conversation-timeline] jump loop", {
                ms: Date.now() - startedAt,
                rows: now,
                idleRounds,
                button: anyBtn ? (anyBtn.disabled ? "disabled" : anyBtn.textContent.trim()) : null,
                key,
                needle: needle.slice(0, 40),
              });
              // 连续无进展且按钮 disabled（宿主 hasMore=false）：已全部加载，停止
              if (idleRounds >= 3 && (!anyBtn || anyBtn.disabled)) break;
            }
          } finally {
            setJumpLoading(false);
          }

          // 兜底：先按用户消息行文本匹配（跳过 meow-memory 等注入的上下文/记忆节点）
          target = findTargetByText();
          if (target) {
            target.scrollIntoView({ behavior: "smooth", block: "start" });
            return;
          }
          ...
          const candidates = Array.from(root.querySelectorAll("*")).filter((el) => {
            ...
            if (!(text.length >= 4 && (text === preview || text.startsWith(preview.slice(0, 40)) || preview.startsWith(text.slice(0, 40))))) return false;
            const row = el.closest("[data-chat-anchor-key]");
            if (row && row.dataset.chatFlowKind && row.dataset.chatFlowKind !== "user") return false;
            return true;
          });
            ...
            inject: () => ({
              loadOlder: (sessionId) => {
                try {
                  ...
                  console.log("[conversation-timeline] loadOlder wrapper error", String((err && err.message) || err));
                  return undefined;
                }
              },
            }),
