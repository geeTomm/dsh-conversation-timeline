window.__ModuleLoader__.load({
  ...
  factory: (require) => {
        ...
        async (ev) => {
          ...
          try {
            ...
            while (Date.now() - startedAt < 15000) {
              ...
              if (btn) {
                // 还有加载按钮：按钮可用就继续触发加载，禁用说明加载中，等待即可
                if (!btn.disabled && typeof loadOlder === "function" && currentId) {
                  try { loadOlder(currentId); } catch (err) { /* 忽略 */ }
                }
              } else {
                // 没有更多加载按钮：再确认一次后停止
                await wait(300);
    ...
    exports.apply = function (ctx) {
      ...
      // 无 better-sidebar 时自动跳过 → 插件退化为纯浮动 overlay，两态兼容。
