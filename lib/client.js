window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function TimelineOverlay({ useSessions, loadOlder, embedded }) {
      ...
      const jumpToUserMessage = useCallback(
        async (ev) => {
          ...
          // 目标节点还没加载：直接调用会话的 loadOlder()，加载到目标出现就停，不做完全加载
          for (let i = 0; i < 40; i++) {
            let more = false;
            if (typeof loadOlder === "function" && currentId) {
              try {
                const result = await loadOlder(currentId);
                more = result !== false && result !== undefined;
              } catch (err) {
                more = false;
              }
            }
    ...
    exports.apply = function (ctx) {
      ...
      // 注册进 dsh-better-sidebar 侧边栏（如已装配）；保留原浮动 overlay 双入口
      const bs = ctx.betterSidebar || (typeof ctx.get === "function" ? ctx.get("betterSidebar") : void 0);
