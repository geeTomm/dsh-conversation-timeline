...
function dialogueText(event) {
  ...
  switch (event.type) {
    case "user/message":
      ...
      const text = contentText(data.content, false)
      // 兼容 meow-memory：忽略记忆注入/回填风格的整段文本，避免干扰时间线与跳转。
      if (/^可能相关的记忆|^相关记忆|^【记忆|meow-memory|记忆，仅供参考/i.test(text)) return ""
      return text
...
`)
        ...
        // 双模式：优先使用 FTS searchSessions；若部署关闭了 FTS（openAt: never），自动回退到逐会话扫描。
        if (req.method === "GET" && pathname === `${API_PREFIX}/search`) {
          ...
          const eventFilters = includeTools ? undefined : [{ kind: "type", values: ["user/message", "assistant/message"] }]

          let sessions = null
          let ftsUsed = false
          try {
            const page = await ctx.sessionQuery.searchSessions({ query: q, eventFilters, limit: 50 })
            sessions = page.items
            ftsUsed = true
          } catch (err) {
            ctx.logger?.info?.("[conversation-timeline] FTS unavailable, fallback to scan: " + String(err))
            sessions = null
          }
          if (!sessions) {
            sessions = await ctx.sessionQuery.listSessions()
          }
          ...
          for (const s of sessions) {
            try {
              ...
              ctx.logger?.warn?.("[conversation-timeline] search session failed: " + s.header.id + " " + String(err))
            }
          }
